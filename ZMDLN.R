...
#mass probability function of zero modified dis_log_normal
...
zdlnorm<-function(p,mu,sigma,y) ifelse(y==1, p+(1-p)*ddlnorm(mu,sigma,y), (1-p)*ddlnorm(mu,sigma,y)) 
...
for(j in 1:d2){
  ...
  estlz = optim(c(0.03,mean(log(xx)),sd(log(xx))),fun2,control=list(fnscale=-1),y=xx,hessian=TRUE)  
...
print(DD)
write.csv(DD,file="Result_ZMLN.csv")
